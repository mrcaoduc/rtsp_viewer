...
class PortScannerApp:
    def __init__(self, master):
        ...
        self.ip_scan_results = {} # Lưu trữ kết quả tổng hợp cho mỗi IP
    ...
    def create_widgets(self):
        ...
        # Đã sửa các cột để hiển thị tổng hợp theo IP
        self.results_tree = ttk.Treeview(self.master, columns=("ip", "open_tcp_ports", "open_udp_ports", "ip_progress"), show="headings")
        self.results_tree.heading("ip", text="IP", anchor=tk.W)
        self.results_tree.heading("open_tcp_ports", text="Cổng TCP Mở", anchor=tk.W)
        self.results_tree.heading("open_udp_ports", text="Cổng UDP Mở", anchor=tk.W)
        self.results_tree.heading("ip_progress", text="Tiến độ IP", anchor=tk.W)

        self.results_tree.column("ip", width=120, stretch=tk.NO)
        self.results_tree.column("open_tcp_ports", width=300, stretch=tk.YES)
        self.results_tree.column("open_udp_ports", width=300, stretch=tk.YES)
        self.results_tree.column("ip_progress", width=100, stretch=tk.NO)
    ...
    def clear_results(self):
        ...
        self.ip_scan_results = {} # Reset kết quả tổng hợp
    ...
    def add_result_to_tree(self, ip, port, status, rtt, banner):
        """
        Không sử dụng trực tiếp hàm này nữa.
        Thay vào đó, sử dụng _update_ip_row_in_tree.
        """
        pass

    def _update_ip_row_in_tree(self, ip, ip_data):
        """Cập nhật hoặc thêm hàng IP vào Treeview với kết quả tổng hợp."""
        item_id = ip_data["tree_item_id"]
        
        # Định dạng chuỗi cổng mở
        tcp_ports_str = ", ".join(ip_data["open_tcp"]) if ip_data["open_tcp"] else "Không có"
        udp_ports_str = ", ".join(ip_data["open_udp"]) if ip_data["open_udp"] else "Không có"

        # Cập nhật tiến độ IP
        ip_progress_text = f"{ip_data['scanned_count']}/{ip_data['total_ports']} Cổng"
        if ip_data['scanned_count'] == ip_data['total_ports']:
            ip_progress_text = "Hoàn tất"

        if item_id is None:
            # Nếu chưa có hàng cho IP này, hãy tạo nó
            ...
            ip_data["tree_item_id"] = item_id # Lưu item_id vào dữ liệu IP
        else:
            # Nếu đã có hàng, cập nhật các giá trị
    ...
    def _run_scan_in_thread(self, ip_start, ip_end, port_start, port_end, timeout):
        ...
        try:
            ...
            self.master.after(0, lambda: self.update_status("Quét đã bị hủy.", is_error=True))
        except Exception as e:
            self.master.after(0, lambda: self.update_status(f"Lỗi không mong muốn trong quá trình quét: {e}", is_error=True))
        finally:
    ...
    async def _scan_ip_and_ports_async(self, ip_start_obj, ip_end_obj, port_start, port_end, timeout):
        ...
        all_ips = []
        current_ip_addr = ip_start_obj
        while current_ip_addr <= ip_end_obj:
            all_ips.append(str(current_ip_addr))
            current_ip_addr += 1

        total_ports_to_scan = len(all_ips) * (port_end - port_start + 1)
        scanned_ports_count_global = 0

        self.master.after(0, self.update_status, f"Bắt đầu quét {total_ports_to_scan} cổng trên {len(all_ips)} IP...")

        # Khởi tạo dữ liệu cho mỗi IP trước khi quét
        for ip_str in all_ips:
            self.ip_scan_results[ip_str] = {
                "open_tcp": [],
                "open_udp": [],
                "scanned_count": 0,
                "total_ports": (port_end - port_start + 1),
                "tree_item_id": None # ID của hàng trong Treeview
            }
            # Thêm hàng ban đầu vào treeview với trạng thái "Đang quét..."
            self.master.after(0, self._update_ip_row_in_tree, ip_str, self.ip_scan_results[ip_str])
        ...
        for ip_str in all_ips:
            for port in range(port_start, port_end + 1):
                ...
                if scan_type == "TCP_CONNECT":
                    task_coroutine = tcp_connect_scan(ip_str, port, timeout)
                elif scan_type == "TCP_SYN" and SCAPY_AVAILABLE:
                    task_coroutine = tcp_syn_scan(ip_str, port, timeout)
                elif scan_type == "UDP" and SCAPY_AVAILABLE:
                    task_coroutine = udp_scan(ip_str, port, timeout)
                else:
                    # Fallback hoặc thông báo lỗi nếu Scapy không có nhưng loại quét yêu cầu Scapy
                    self.master.after(0, self.update_status, "Lỗi: Quét không khả dụng (Cần Scapy/Admin)", is_error=True)
                    continue
                ...
                if len(tasks) >= 50: # Giới hạn số lượng tác vụ đồng thời để tránh quá tải
                    ...
                    for completed_task in done:
                        ...
                        scanned_ports_count_global += 1

                        ip_data = self.ip_scan_results[ip]
                        ip_data["scanned_count"] += 1

                        if status == "Mở":
                            port_info = f"{port_num}"
                            if rtt != "N/A":
                                port_info += f" ({rtt}"
                                if do_banner_grab and banner_result:
                                    port_info += f" - {banner_result}"
                                port_info += ")"
                            elif do_banner_grab and banner_result:
                                port_info += f" ({banner_result})"

                            if scan_type == "UDP": # UDP Scan
                                ip_data["open_udp"].append(port_info)
                            else: # TCP Connect/SYN Scan
                                ip_data["open_tcp"].append(port_info)
                        
                        self.master.after(0, self._update_ip_row_in_tree, ip, ip_data)
                        self.master.after(0, self.update_progress, scanned_ports_count_global, total_ports_to_scan)
        ...
        self.master.after(0, self.update_status, f"Quét hoàn tất. Đã quét {scanned_ports_count_global}/{total_ports_to_scan} cổng.")
...
if __name__ == "__main__":
    ...
    # Điều này quan trọng nếu ứng dụng bị đóng đột nhiên trong khi quét đang chạy
