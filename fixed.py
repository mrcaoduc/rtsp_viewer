    ...
    def resizeEvent(self, event):
        if not self.is_fullscreen:
            try:
                ...
                print(f"Error in CustomLabel resizeEvent: {str(e)}")
...
class RTSPViewer(QMainWindow):
    def __init__(self):
        ...
        self.max_streams = 8
        self.grid_size = 2
        self.vlc_instance = vlc.Instance('--no-xlib --verbose=2 --network-caching=500 --rtsp-caching=500 --live-caching=500 '
                                         '--no-video-title-show --clock-jitter=0 --clock-synchro=0 '
                                         '--rtsp-frame-buffer-size=10000 --rtsp-timeout=10 --aout=dummy --no-audio')
        self.workers = []
        self.updating = False
    ...
    def start_stream_update(self, urls_text):
        try:
            ...
            for i in range(required_labels):
                ...
                worker.finished.connect(lambda: self.worker_finished(i))
    ...
    def update_grid_layout(self):
        ...
        try:
            ...
            cell_width = min(window_size.width() // self.grid_size, max_width // self.grid_size)
            cell_height = min(int(cell_width / self.labels[0].aspect_ratio) if self.labels else window_size.height() // self.grid_size, max_height // self.grid_size)
            
            for i, label in enumerate(self.labels):
                row = i // self.grid_size
                col = i % self.grid_size
                label.setFixedSize(cell_width, cell_height)
                self.grid_layout.addWidget(label, row, col, 1, 1, Qt.AlignCenter)
    ...
    def deferred_resize(self):
        try:
            if not self.updating:
                ...
                screen = QDesktopWidget().screenGeometry()
                max_width, max_height = screen.width(), screen.height() - 50
                new_size.setWidth(min(new_size.width(), max_width))
                new_size.setHeight(min(new_size.height(), max_height))
                
                scale_width = new_size.width() / self.last_size.width() if self.last_size.width() > 0 else 1.0
                scale_height = new_size.height() / self.last_size.height() if self.last_size.height() > 0 else 1.0
                scale = min(scale_width, scale_height)
                
                for label in self.labels:
                    if not label.is_fullscreen:
                        current_size = label.size()
                        new_width = int(current_size.width() * scale)
                        new_height = int(new_width / label.aspect_ratio)
                        label.setFixedSize(new_width, new_height)
                
                self.last_size = new_size
                self.update_grid_layout()
        except Exception as e:
            print(f"Error in deferred_resize: {str(e)}")
